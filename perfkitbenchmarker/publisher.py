...
class PushgatewayPublisher(SamplePublisher):
  ...
  def __init__(self, pushgateway):
    ...
    self.documentations = {
      ...
      ('mesh_network', self.toSnakeCase('TCP_RR_Average_Latency')): 'TCP Request/Response avg. latency b/w this pod and all the other ones',
      ('mesh_network', self.toSnakeCase('TCP_STREAM_Total_Throughput')): '10 sec. long total TCP stream throughput b/w this pod and all the other ones',
      ...
      ('iperf', self.toSnakeCase('Throughput')): 'TCP payload throughput between the two ends',
      ('iperf', self.toSnakeCase('lscpu')): 'Information about the CPU architecture',
      # ('iperf', self.toSnakeCase('proccpu')): '',
      # ('iperf', self.toSnakeCase('proccpu_mapping')): '',
      # ('iperf', self.toSnakeCase('End to End Runtime')): '',
    }
  ...
  def toSnakeCase(self, input_string, unit=None):
    ...
    result = input_string.replace(" ", "_").replace("(", "").replace(")", "").replace("/", "_").replace(";", "_").\
  ...
  def PublishSamples(self, samples):
    ...
    if self.pushgateway is not None:
      ...
      for sample in samples:
        ...
        push_to_gateway(gateway=self.pushgateway, job=self.job_label_prefix + sample['test'], registry=self.registry)
